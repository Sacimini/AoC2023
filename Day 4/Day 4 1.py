  ...
  if len(parsedScratchCardGame) > 0:
    ...
    for cardNumber in cardNumbers:
      if cardNumber in winningNumbers and cardNumber != '':
        ...
        continue
  if cardValue > 0:
    print(f'Card #{cardID} contains {cardValue} winners worth {2**(cardValue-1)}')        
    totalScatchCardPoints += 2**(cardValue-1)

print(f'The {len(scratchCards)} cards and worth {totalScatchCardPoints} points')