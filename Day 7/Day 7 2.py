...
#    print(f'Original Hand : {gameHand} Sorted : {sortedGameHand}')
    ...
    cardCounts = sorted(cardGameHistogram.items(), key=lambda x:(x[1], x[0]), reverse=True)
    for cardCount in cardCounts:
      if cardCount[0] != 'J':
        handWithJokersApplied = sortedGameHand.replace('J', cardCount[0])
        break
      elif cardCount[1] == 5:
        handWithJokersApplied = sortedGameHand.replace('J', '2')
    cardGameHistogram = dict((index, handWithJokersApplied.count(index)) for index in handWithJokersApplied)
    cardGameResult = 'HIGH_CARD'
    for playingCard in cardGameHistogram:
      if cardGameHistogram[playingCard] == 5:
        cardGameResult = 'FIVE_OF_A_KIND'
      elif cardGameHistogram[playingCard] == 4:
        cardGameResult = 'FOUR_OF_A_KIND'
      elif cardGameHistogram[playingCard] == 3:
        if cardGameResult == 'ONE_PAIR':
          cardGameResult = 'FULL_HOUSE'
        else:
          cardGameResult = 'THREE_OF_A_KIND'
      elif cardGameHistogram[playingCard] == 2:
    ...
    camelCardGameResults.append({'Game': index, 'Hand': gameHand, 'cardValue': gameHand.replace('A', 'E').replace('K', 'D').replace('Q', 'C').replace('J', '0').replace('T', 'A'), 'Sorted': ''.join(sorted(gameHand)), 'JokersApplied' : handWithJokersApplied, 'Bid': gameBid, 'Result': cardGameResult, 'Rank': camelCardWinningHands[cardGameResult] })
...
for index, rankedGameResult in enumerate(camelCardGameResults, start = 1):
  ...
  print(f'Hand {rankedGameResult["Hand"]} Sorted Hand {rankedGameResult["Sorted"]} Jokers Applied {rankedGameResult["JokersApplied"]} Bid {rankedGameResult["Bid"]} Result {rankedGameResult["Result"]} Multipler {index} Total {rankedGameResult["Bid"]} * {index}  Result {rankedGameResult["Bid"] * index} Running_Total {totalCamelCardWinnings}')
